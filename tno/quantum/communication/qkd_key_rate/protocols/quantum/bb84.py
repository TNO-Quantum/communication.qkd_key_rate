...
def ensure_probability(p: float) -> float:
...
def lower_bound_matrix_gain(max_num_photons: int, mus: Union[List, ArrayLike]) -> float:
...
def bound_f(number_of_pulses: int, probability: float, epsilon: float) -> float:
...
class BB84FullyAsymptoticKeyRateEstimate(AsymptoticKeyRateEstimate):
    ...
    def __init__(self, detector: Detector, **kwargs) -> None:
        """
        Args:
...
class BB84AsymptoticKeyRateEstimate(AsymptoticKeyRateEstimate):
    ...
    def __init__(self, detector: Detector, number_of_decoy: int = 2, **kwargs) -> None:
        """
        Args:
            detector:  The detector used at Bob's side
