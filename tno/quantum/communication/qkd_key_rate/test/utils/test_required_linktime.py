...
from tno.quantum.communication.qkd_key_rate.utils.required_linktime import (
    get_efficiency,
    get_estimate_on_communication,
)
...
def test_required_linktime(
    ...
    number_of_pulses = np.array(
        [factor * 10**exp for exp in [9, 10, 11] for factor in range(1, 10)]
    )
    ...
    number_of_communication_rounds = get_estimate_on_communication(
        error_correction_method, error_estimate, message_length
    )

    ec_efficiency = get_efficiency(error_correction_method, error_estimate)
...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    ax.loglog(number_of_pulses, entropy)
