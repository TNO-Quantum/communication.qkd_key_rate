...
def test_invalid_interval_field() -> None:
    ...
    with pytest.raises(AssertionError):
        # interval != 1 / detection_frequency
        Detector(
            name="standard",
            efficiency_detector=0.2,
            jitter_source=0,
...
def test_invalid_dark_count_rate_field() -> None:
    ...
    with pytest.raises(AssertionError):
        # dark_count_frequency != dark_count_rate * detection_frequency
        Detector(
            name="standard",
            efficiency_detector=0.2,
            jitter_source=0,
...
def test_invalid_efficiency_field() -> None:
    ...
    with pytest.raises(AssertionError):
        # efficiency_party !=  efficiency_system * efficiency_detector
        Detector(
            name="standard",
            jitter_source=0,
            jitter_detector=5.00e-11,
...
def test_from_file() -> None:
    ...
    path = "tno/quantum/communication/qkd_key_rate/test/base/detectors.csv"
